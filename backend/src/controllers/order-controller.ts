...
export const handleCreateWholesaleRazorpayOrder = async (req: any, res: any) => {

    try {

        const { warehouseInventoryId, quantity } = createWholesaleRazorpayOrderSchema.parse(req.body);



        const wItem = await db.query.warehouseInventory.findFirst({
        ...
        if (availableStock < quantity) {
            ...
            return res.status(400).json({ message: `Insufficient wholesale stock. Available: ${availableStock}` });

        }



        let totalAmount = parseFloat(wItem.price) * quantity;
        ...
        const options = {
            ...
            receipt: `receipt_wholesale_order_${new Date().getTime()}`,
        ...
        console.error('Error creating wholesale Razorpay order:', e);
...
export const handleVerifyWholesalePayment = async (req: any, res: any) => {
    ...
    try {
        ...
        } = verifyWholesalePaymentSchema.parse(req.body);
        ...
        // Payment is verified, now create the wholesale order
        ...
        const result = await db.transaction(async (tx) => {
            ...
            // Add to shop inventory directly
            ...
            if (existingShopItem) {
                ...
                await tx.update(shopInventory)
                    ...
                    .set({
                        ...
                        price: shopPrice ? shopPrice.toString() : existingShopItem.price,
                ...
                await tx.insert(shopInventory).values({
                    ...
                    price: shopPrice.toString(), // Use the price set by retailer
        ...
        console.error('Error verifying wholesale payment:', e);
...
export const handlePostWholesaleOrder = async (req: any, res: any) => {
    ...
    try {
        ...
        const { warehouseInventoryId, quantity, paymentMethod, isProxyItem, shopPrice } = createWholesaleOrderSchema.parse(req.body);



        if(paymentMethod === 'razorpay') {

            return res.status(400).json({ message: "Please use the Razorpay-specific endpoint for this payment method." });
            ...
            if(isProxyItem) {
                ...
                if (existingShopItem) {
                    ...
                    await tx.insert(shopInventory).values({
                        ...
                        price: shopPrice ? shopPrice.toString() : wItem.price,
